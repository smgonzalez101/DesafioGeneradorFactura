...
@WebServlet("/factura") // definimos la ruta a través de la cual accederemos al servlet
public class FacturaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)//
		...
		// capturar cantidad ingresada por el usuario de cada producto
		...
		// Capturar precio unitario de los producto
		...
		if (nombre.equals("") || empresa.equals("") || rut.equals("") || direccion.equals("") || ciudad.equals("")
			...
			request.setAttribute("msjError", "Faltan datos del cliente, ingrese nuevamente...");// Pasar el mensaje de
																								// error al jsp o
																								// servlet
			...
			if (item01 > 0) {
				...
				Producto producto01 = new Producto("Válvulas de titanio", "Válvulas de carrera", 120000, item01,
						valvulaVT);
				listaProductos.add(producto01);
			}
			if (item02 > 0) {
				turboVT = vu02 * item02;
				Producto producto02 = new Producto("Turbo Full Carrera", "Turbo de competición multimarca", 1700000,
						item02, turboVT);
			...
			if (item03 > 0) {
				...
				Producto producto03 = new Producto("Kit de Freno Competición",
						"Juego de discos, balatas, caliper de competición", 760000, item03, kitVT);
				listaProductos.add(producto03);
			}
			if (item04 > 0) {
				sistemaVT = vu04 * item04;
				Producto producto04 = new Producto("Sistema de Refrigeración",
						"Sistema de enfriamiento de motor Carrera", 2300000, item04, sistemaVT);
				listaProductos.add(producto04);
			}
			if (item05 > 0) {
				plumillaVT = vu05 * item05;
				Producto producto05 = new Producto("Plumillas limpia parabrisas estandar", "Plumillas para la lluvia",
						10000, item05, plumillaVT);
			...
			request.getRequestDispatcher("factura.jsp").forward(request, response);// llamado a factura.jsp
		}
		request.getRequestDispatcher("formularioIngreso").forward(request, response);
